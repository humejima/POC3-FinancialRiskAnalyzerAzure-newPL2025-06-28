...
def create_test_data():
    """
    参照マッピング機能をテストするためのデータを作成
    CSVDataとAccountMappingモデルの正確なフィールド名を使用
    """
    with app.app_context():
        ...
        # 既存のマッピングデータがあれば削除
        # AccountMappingには year フィールドがなく financial_statement が file_type に相当
        ...
        for i, std_account in enumerate(standard_accounts):
            ...
            csv_data = CSVData(
                ...
                current_value=1000000 * (i+1),  # value -> current_value
                previous_value=800000 * (i+1),
                is_mapped=False,  # すべて未マッピング状態
                standard_account_code=None  # これは本当はモデルに定義されていない
            )
