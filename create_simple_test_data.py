"""
参照マッピング機能の簡易テストデータを作成するスクリプト
最小限のテストデータを作成することでタイムアウトを回避
"""
...
def create_simple_test_data():
    ...
    with app.app_context():
        ...
        # 既存のデータがあれば削除
        for file_type in ['bs']:  # テスト用に貸借対照表のみに絞る
            CSVData.query.filter_by(ja_code=ja_code, year=year, file_type=file_type).delete()
            AccountMapping.query.filter_by(ja_code=ja_code, year=year, file_type=file_type).delete()
        ...
        for i, csv_data in enumerate(csv_data_list):
            ...
            mapping = AccountMapping(
                ...
                year=year,
                file_type='bs',
                original_name=csv_data.account_name,
                standard_account_code=std_account.code,
                standard_account_id=std_account.id,
                confidence=0.95,
